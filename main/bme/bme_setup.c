...
void delay_us(uint32_t period_us, void *intf_ptr) { vTaskDelay( (period_us/1000) / portTICK_PERIOD_MS); }
 
void print_rslt(const char api_name[], int8_t rslt) {
 
    if (rslt != BME280_OK) {
        if (rslt == BME280_E_NULL_PTR) {
            ESP_LOGE(__func__, "Error [%d] : Null pointer error", rslt);
        } else if (rslt == BME280_E_DEV_NOT_FOUND) {
            ESP_LOGE(__func__, "Error [%d] : Device not found", rslt);
        } else if (rslt == BME280_E_INVALID_LEN) {
            ESP_LOGE(__func__, "Error [%d] : Invalid Lenght", rslt);
        } else if (rslt == BME280_E_COMM_FAIL) {
            ESP_LOGE(__func__, "Error [%d] : Bus communication failed", rslt);
        } else if (rslt == BME280_E_SLEEP_MODE_FAIL) {
            ESP_LOGE(__func__, "Error [%d] : SLEEP_MODE_FAIL", rslt);
        } else {
            /* For more error codes refer "*_defs.h" */
            ESP_LOGE(__func__, "Error [%d] : Unknown error code", rslt);
        }
    }else{
        ESP_LOGI(__func__, " BME280 status [%d]", rslt);
    }
}

void store_data(struct bme280_data *comp_data)
{
    bme_dados.temp = comp_data->temperature;
    bme_dados.umid = comp_data->humidity;
    bme_dados.pres = comp_data->pressure;
