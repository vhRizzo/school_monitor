...
void dsm_task ( void* pvParameters )
{
    ...
    gpio_isr_handler_add(DSM_PIN_VERM, change_v2_isr, (void*) DSM_PIN_VERM);
    gpio_isr_handler_add(DSM_PIN_AMAR, change_v1_isr, (void*) DSM_PIN_AMAR);

    for( ; ; ) {
        vTaskDelay( (TEMPO_ANALISE*1000) / portTICK_PERIOD_MS );

        gpio_isr_handler_remove(DSM_PIN_VERM);
        gpio_isr_handler_remove(DSM_PIN_AMAR);
